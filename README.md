...
Copy files to device. Note that tis project copies over the default main.py file so that other apps are not available, but after a power loss the device will boot back to the controller automatically. If you want to have this be just another button on the default set up, don't copy ``main.py`` to the device before renaming it.

Next change the following fields in ``button_defs.json`` to the correct values for your local setup
* host : this is the local network IP address for the server. If the port is the standard 9000, no additional argument is needed. If a non-default was use in the server setup, then that new value needs to be added into the JSON file here as ``"port":xxxx``
* player : This is the player name that this controller is tied to
* timezone : Pick your local timezone from the list at http://worldtimeapi.org/timezones. This will not only determine the correct time, but also when the clock heeds to be reset for daylight savings time, if its donn in that time zone.
